...
class TrackVideo {
    ...
    // Frontend video tracker
    public function enqueue_scripts() {
        if (!is_user_logged_in() || !is_page('pre-membership-education-seminar')) return;
        ...
        wp_localize_script('video-tracker', 'VideoTracker', [
    ...
    public function enqueue_admin_scripts($hook) {
        ...
        wp_localize_script('pmes-admin', 'PMESAdmin', [
            'ajaxurl' => admin_url('admin-ajax.php'),
            'nonce'   => wp_create_nonce('pmes_reset_nonce'),
        ]);
    }

    private function verify_nonce($nonce_name = 'video_tracker_nonce') {
    ...
    private function increment_counter($type) {
        $user_id = get_current_user_id();
        if (!$user_id) wp_send_json_error(['message' => 'Invalid request']);

        $data = get_user_meta($user_id, 'pmes_video_progress', true);
        if (!is_array($data)) {
            $data = ['played' => 0, 'paused' => 0, 'skipped' => 0, 'completed' => 0, 'watched_seconds' => 0];
        }
    ...
    public function handle_video_completed() {
        ...
        if (empty($progress['completed'])) $this->trigger_n8n_workflow();
    ...
    private function trigger_n8n_workflow() {
        ...
        if (is_wp_error($response)) error_log('PMES n8n webhook failed: ' . $response->get_error_message());
    }

    public function display_video_progress($user): void {
        $user_id = $user->ID;
        $progress = get_user_meta($user_id, 'pmes_video_progress', true);
        $progress = is_array($progress) ? $progress : ['played'=>0,'paused'=>0,'skipped'=>0,'completed'=>0,'watched_seconds'=>0];

        // Calculate percent based on seconds watched (assumes 600s video)
        $percent = isset($progress['watched_seconds']) ? min(100, round($progress['watched_seconds'] / 600 * 100)) : 0;
        ?>
        <h2>PMES Video Progress</h2>
        <table class="form-table" id="pmes-progress-table-<?php echo esc_attr($user_id); ?>">
            <tr><th>Progress</th><td><?php echo esc_html($percent); ?>%</td></tr>
            <tr><th>Times Played</th><td><?php echo esc_html($progress['played']); ?></td></tr>
            <tr><th>Times Paused</th><td><?php echo esc_html($progress['paused']); ?></td></tr>
            <tr><th>Times Skipped</th><td><?php echo esc_html($progress['skipped']); ?></td></tr>
            <tr><th>Status</th><td><?php echo !empty($progress['completed']) ? 'Completed ✅' : 'Not Completed ❌'; ?></td></tr>
            <tr>
                <th>Reset Progress</th>
                <td>
                    <button class="button button-secondary pmes-reset-btn" data-user-id="<?php echo esc_attr($user_id); ?>">Reset Progress</button>
