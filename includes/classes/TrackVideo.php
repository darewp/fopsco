...
class TrackVideo {
    ...
    private function increment_counter($type, $watched_seconds = 0, $video_duration = 0) {
        ...
        if (!is_array($data)) {
            $data = ['played'=>0,'paused'=>0,'skipped'=>0,'completed'=>0,'watched_seconds'=>0,'video_duration'=>0];
        }

        if (isset($data[$type])) $data[$type]++;
        if ($watched_seconds) $data['watched_seconds'] = $watched_seconds;
        if ($video_duration)  $data['video_duration']  = $video_duration;
    ...
    private function verify_nonce() {
        if (!isset($_POST['_ajax_nonce']) || !wp_verify_nonce($_POST['_ajax_nonce'],'video_tracker_nonce')) {
    ...
    public function display_video_progress($user): void {
        $user_id = $user->ID;
        $progress = get_user_meta($user_id,'pmes_video_progress',true);
        $progress = is_array($progress) ? $progress : ['played'=>0,'paused'=>0,'skipped'=>0,'completed'=>0,'watched_seconds'=>0,'video_duration'=>0];

        $percent = 0;
        if (!empty($progress['video_duration'])) {
            $percent = min(100, round($progress['watched_seconds'] / $progress['video_duration'] * 100));
        }
        ?>
